...
(function () {
  ...
  function buildManifest(submissionId, details, now) {
    ...
    return {
      ...
      generated_by: "Indigenize Toys pilot portal (generated in the browser, nothing uploaded)"
    };
  }
  ...
  async function downloadPackage() {
    ...
    try {
      ...
      $("result-text").textContent = "Saved as " + filename + " (" + formatBytes(blob.size) + "). " +
        "Now email this file to jon@indigenize.toys as an attachment. " +
        "If it is too large for email, share it through Google Drive and send the link.";
      $("result").hidden = false;
